...
function cut2(e){ e.classList.toggle('cutact'); var p=e.nextSibling; p.style.maxHeight=(p.style.maxHeight?null:p.scrollHeight+'px'); }
");
STYLES("cut",".cut,.cutnc{cursor:pointer;color:blue;}.cut{text-align:center}.cut:hover,.cutnc:hover{text-decoration:underline;}
.cut2 { background-color: #CFD8DC; cursor: pointer; width:100%; border:none; text-align:left; outline:none; transition:0.5s; }
.cutact, .cut2:hover { background-color: #607D8B; }
.cut2:before { content: '\\002B '; font-size: 18px; font-weight:bold; margin-right:5px; }
.cutact:before { content: '\\2212 '; }
.cut2p { padding: 0 18px; color: #555; background-color: #fff; max-height: 0; overflow: hidden; transition: max-height 0.3s ease-out; }
");
...
'template_div'=>"<div class='cut2' onclick='cut2(this)'>{click}</div><div class='cut2p'>{text}</div>",
// <div class='{cut}' onclick='cut(this)'>{click}</div><div style='display:none'>{text}</div>",
