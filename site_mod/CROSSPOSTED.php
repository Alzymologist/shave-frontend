...
telegraph_view=function(){
    doclass('telegraph',function(e){
      e=e.querySelector('A');
      AJ(e.href.replace(/https\:\/\/telegra\.ph\//g,'https://api.telegra.ph/getViews/'),function(s){
...
'tmpl'=>"<div class='{net}'><i>{adminkeys}<a href='{#link}'>{#link}</a></i></div>",
...
$r=ms("SELECT `net`,`id` FROM `socialmedias` WHERE `num`=".intval($num)." AND `type`='post'".ANDC(),"_a",0); if(!sizeof($r)) return "";
