...
list($article["Year"],$article["Mon"],$article["Day"])=(substr_count($article['Date'],'/')>1?@explode("/",substr($article['Date'],0,10),3):array('','',''));
...
$myDD=@intval($article['DateDatetime']);
list($article['Prev'],$article['PrevHeader'])=ms("SELECT `Date` as '0',`Header` as '1' FROM `dnevnik_zapisi` ".WHERE("`DateDatetime`<".$myDD." AND `DateDatetime`!=0")." ORDER BY `DateDatetime` DESC LIMIT 1","_1");
list($article['Next'],$article['NextHeader'])=ms("SELECT `Date` as '0',`Header` as '1' FROM `dnevnik_zapisi` ".WHERE("`DateDatetime`>".$myDD)." ORDER BY `DateDatetime` LIMIT 1","_1");
return ''; }
...
// MAIL('Mail.write({_user_unic:_})')
    if(empty($e)) $e="<div id='logindiv' style='display:inline;position:absolute;z-index:3;top:5px;right:10px;font-size:9px;'>"
."<img onclick=\"{onclick}\" onerror=\"this.src=www_design+'img/login.png';\" alt=\"{imgicourl}\" style=\"cursor:pointer;height:43px !important;width:43px !important;object-fit:cover;border-radius:50%;\" src=\"{userpic_url}\">"
."<span style=\"bottom:5px;height:10px;position:absolute;right:0;width:10px;\">{zamok}</span>"
."<span alt='личная почта' onclick=\"MAIL('Mail.All()')\" style=\"cursor:pointer;bottom:25px;right:-5px;position:absolute;\">"
."<i class='e_kmail'></i>"
."{_MAILBOX:<span class='mail_count' style=\"background:#dc4666;border-radius:50%;color:#fff;font-size:10px;text-align:center;padding:3px;\">"."</span>_}"
."</span>"
."</div>"
."<script>if(window.top!==window) document.getElementById('logindiv').style.display='none';</script>"
;
...
$c=array_merge((empty($GLOBALS['IS'])?array(
    'id'=>'',
    'loginlevel'=>0,
    'zamok'=>'',
    'imgicourl'=>0
	...
	'onclick_nocard'=>"ifhelpc(mypage.split('://')[0]+'://'+ MYHOST+'/login','logz','Login')",
	...
	$c['userpic_url']=(empty($c['id'])?'{wwwhost}design/img/login.png':'{wwwhost}user/{id}/userpick.jpg');
...
function ANOTHER_DATE() { global $article; $s='';
    if($article['DateDate']) {
	$pp=ms("SELECT `Date`,`Header` FROM `dnevnik_zapisi` ".WHERE("`DateDate`=".intval($article['DateDate'])." AND `Date`!='".e($article['Date'])."' ".ANDC()),"_a");
