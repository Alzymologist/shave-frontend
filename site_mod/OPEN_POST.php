...
function OPEN_POST_ajax() { if(!$GLOBALS['podzamok']) return '';
	$num=RE('num');
	if(($sql=ms("SELECT `Body`,`Access` FROM `dnevnik_zapisi` WHERE `num`=".intval($num).ANDC(),'_1',0))==false) return "alert('Error #1')";
...
function OPEN_POST_action($num,$a) {
	...
	msq_update('dnevnik_zapisi',array('Access'=>$a),"WHERE `num`=".intval($num).ANDC());
