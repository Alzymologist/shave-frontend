...
function getlastcom() { global $lim,$mode,$lastcom,$ncom; if(isset($lastcom)) return;
    ...
    if(!$l && !$mode) $l=ms("SELECT `time` FROM `lastcomm` WHERE `unic`=".intval($GLOBALS['unic'])." AND `acn`=".intval($GLOBALS['acn']),"_l",0); // вз€ть из хранилища

    // если дата и там не задана, вз€ть текущее врем€
    if(!$l) $l=time();

    if(!$mode) {
	    if($GLOBALS['unic']==4) { $GLOBALS['aharu']=1; llog(date("Y-m-d H:i:s getlastcom.php")); }
	    msq_add_update('lastcomm',array('time'=>$l,'unic'=>$GLOBALS['unic'],'acn'=>$GLOBALS['acn']),'unic acn'); // запомнить последние параметры
    }
