...
// ќтображение статьи с каментами - дата передана в $Date
...
$GLOBALS['IMG_TMPL']="{_IMG: {img} _}";
	...
	if($q&&$unic
                && !$GLOBALS['msq_noa'] // если запрещено в конфиге $msq_noa=1; - то ничего не делать
		&& !$GLOBALS['ahtung'] // если нагрузка больша€ - ничего не делать
		&& $p['num'] // и если это не служебна€ страница
	) {
		// $msqe_old=$GLOBALS['msqe']; // запомним накопленные ошибки
		// $GLOBALS['msqe']='';
		msq_add("dnevnik_posetil",array('unic'=>$unic,'url'=>$p['num'],'date'=>time()),1); // если есть - не внесет, а даст ошибку, нам не важно
...
//---------- PRAVKI ----------------
if($GLOBALS['unic'] && isset($GLOBALS['db_pravka'])) {
$ee=ms("SELECT `text`,`textnew` FROM `".$GLOBALS['db_pravka']."` WHERE `unic`=".intval($GLOBALS['unic'])."
AND `metka`='new'
...
if($p['autoformat']!='no') {
 ...
 if($p['autoformat']=='md') { // MarkDown
    ...
    $s=preg_replace("/<\/blockquote>\s*<blockquote>/si",'',$s);

    // Paragraph
    $s=preg_replace("/\n[\t ]*\d+\.[\t ]+([^\n]+)/si","\n<ol><li>$1</li></ol>","\n".$s);
    $s=preg_replace("/<\/li><\/ol>\s*<ol><li>/si","</li><li>",$s);

    // Bold / Itallic
    $s=preg_replace("/\*\*([^\*]+)\*\*/si","<strong>$1</strong>",$s);
    $s=preg_replace("/\_\_([^\_]+)\_\_/si","<strong>$1</strong>",$s);
    $s=preg_replace("/ \*([^\*]+)\*/si"," <em>$1</em>",$s);
    $s=preg_replace("/ \_([^\_]+)\_/si"," <em>$1</em>",$s);

    // Links
    $s=preg_replace("/\[([^\]]+)\]\((http[^\)]+)\)/si","<a href='$2'>$1</a>",$s);
...
"\n".$ots.$ots.$ots.$ots.$ots.$ots.$ots.$ots.$ots,
    ...
    // вычистить дублирующиес€ через пробел линки, образующиес€ после конструкций вида <a href=LINK>LINK</a>
    $s=preg_replace_callback("|(https*\:\/\/[^\s]+) (https*\:\/\/[^\s]+)|",function($t) { return ($t[1]==$t[2]?$t[1]:$t[0]); },$s);
