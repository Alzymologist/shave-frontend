...
// idie($GLOBALS['file_template']);
	...
	if(isset($GLOBALS['comment_template'])) $tmpl=$GLOBALS['comment_template']; // если уже готов темплейт
	    ...
	    if(empty($GLOBALS['comment_tmpl'])) $GLOBALS['comment_tmpl']="comment_tmpl.htm";

	$tmn=rpath(h($GLOBALS['comment_tmpl']));
	$tmpl=get_sys_tmp("comm/".$tmn);

	//    $tmpl=$GLOBALS['comment_template']=fileget($GLOBALS['file_template']."comm/".(empty($GLOBALS['comment_tmpl'])?"comment_tmpl.htm":h($GLOBALS['comment_tmpl'])));

	}

// $tmpl='3333333333333333';
...
function comment_prep($p,$mojno_comm,$level) { global $ADM,$unic,$podzamok,$geoip_color;
    ...
    if($GLOBALS['podzamok'] && !isset($p['time_reg']) && $p['unic']!=0) $p['time_reg']=ms("SELECT `time_reg` FROM ".$GLOBALS['db_unic']." WHERE `id`=".intval($p['unic']),"_l");
...
//	$p['whois_small'] = ($p['strana']?search_podsveti(hh($p['strana'])):'').($p['gorod']?($p['strana']?", ":'').search_podsveti(hh($p['gorod'])):'');
	...
	$p['my']=($unic==$p['unic']?1:0); // мой собственный коммент
	$p['Parent_my']=(!empty($p['Parent_unic']) && $p['Parent_unic']==$unic?1:0); // ответ мне
...
or ($p['my'] and ( // или это твой (посетител€) комментарий, и ...
...
if(stristr($text,'{screen:') or stristr($text,'{scr:')) {
	...
	if(!$ADM && c($text)=='') return false;
		...
		// {username}
		$text=str_replace("{username}","<b>".$GLOBALS['imgicourl']."</b>",$text); // nl2br($text);
...
$sql=ms("SELECT id,unic,Name,Text,Parent,Time,whois,rul,ans,golos_plu,golos_min,scr,DateID,BRO,IPN FROM `dnevnik_comm` WHERE `DateID`=".intval($num)." ORDER BY `Time`","_a",0);
if(!sizeof($sql)) return false;

// чтоб не загружать сложными запросами mysql сука бл€ть нежную кретины бл€ть криворукие написать нормально не могли, теперь мы еще возьмем недостающие значени€ из db_unic
$SQ=array_unique(array_column($sql,'unic'));
if(($k=array_search('0',$SQ)) !== false) unset($SQ[$k]); // удалим комменты 0
$ON_db_unic=array();
if(sizeof($SQ)) {
    $SQ=e(implode(',',$SQ));
    $SQ=ms("SELECT id,capchakarma,mail,admin,realname,login,openid,img,time_reg FROM ".$GLOBALS['db_unic']." WHERE `id` IN (".e($SQ).")",'_a',0);
    foreach($SQ as $e) $ON_db_unic[$e['id']]=get_ISi($e);
}
	$kstop=10000; $comc=array(); $comindex=array();

	foreach($sql as $p) {
		if(isset($ON_db_unic[$p['unic']])) $p=array_merge($ON_db_unic[$p['unic']],$p); // $p=get_ISi($p);
		$comc[$p['id']]=$p;
