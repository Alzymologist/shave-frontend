...
function add_mtime($s) {
    if(strstr($s,'://')) return $s; // не файл
    if(false!==($p=strpos($s,'?'))) $s=substr($s,0,$p); // убрать нахуй "?..." если оно было для файла
...
function modul_n($t) { global $arap;
    if(isset($arap[$t[1]])) {
	if(false===strpos($arap[$t[1]],$t[0])) return $arap[$t[1]]; // если не зациклен тэг
...
function modul_hun($t) { global $arap;
	if(isset($arap[$t[2]])) { $s=$arap[$t[2]]; switch($t[1]) { // если есть данные
		case '#': // str_replace(array('&','<','>'),array('&amp;','&lt;','&gt;'),$s); // экранировать HTML-сущности только <>& {#:<жопа>} --> &lt;жопа&gt;// return h($s); 
		case 'njs': return njs($s); // экранировать кавычки для JS
		case 'url': return urlencode($s); // экранировать url
		case 'wurl': return urlencode(wu($s)); // экранировать url и перекодировать из 1251 в UTF-8
		...
		case 'length': return strlen($s); // число символов в тексте
		case 'nl2br': return nl2br($s); // \n в <br/>
		case '#nl2br': return nl2br(h($s)); // \n в <br\> и еще экранировать HTML-сущности
		case 'date': return date("Y-m-d H:i:s",$s); // число в дату
	}
		if('0'==$t[1][0]) return sprintf("%".$t[1]."d",$s);  // если первый символ 0, то напечатать как число с нужным количеством знаков {06:id} 000012
		...
		case 'div': $a=explode('-',$s); return ($a[0]/$a[1]); // деление {divm:{unic}-100}
		case 'divm': $a=explode('-',$s); return ($a[0]%$a[1]); // остаток от деления {divm:{unic}-100}
	}
...
//		case 'divm100': return "(100)"; // {#:<жопа>} --> &lt;жопа&gt;
...
function domd($s) { // подцепить модули
    if(false!==strpos($s,':')) { // подключаемый модуль
                list($mod,$arg)=explode(':',$s,2); $mod=rpath(c($mod));
        if(in_array($mod,$GLOBALS['norepeat_modules'])) { // некоторым модулям запрещено повторяться больше 1 раза
            if(@($GLOBALS['norepeat_modules'][$mod]==1)) return "<font color=red>MODULE \"$mod\" can not repeat!</font>";
            $GLOBALS['norepeat_modules'][$mod]=1;
