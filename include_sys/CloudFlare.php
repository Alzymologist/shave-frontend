...
$dyndns_clowdflare_token='28478469e80651d6a423f72f8ae01e9db8d34a7';
...
function cloudflare_del_cache($file) { global $ClaudFlare_DOMAINS;
/*
curl -X DELETE "https://api.cloudflare.com/client/v4/zones/${ZONE}/purge_cache" \
     -H "X-Auth-Email: $MAIL" \
     -H "X-Auth-Key: $KEY" \
     -H "Content-Type: application/json" \
     --data '{"files":["http://lleo.me/dnevnik/user/4/userpick000.jpg","http://lleo.me/dnevnik/2019/12/0.jpg"]}'
*/



    if(substr($file,0,strlen($GLOBALS['filehost']))==$GLOBALS['filehost']) $file=substr($file,strlen($GLOBALS['filehost']));
    if(!strstr($file,'://')) $file=$GLOBALS['httphost'].$file;
    if(!isset($ClaudFlare_DOMAINS)) cloud_get_domains();
    if(!isset($ClaudFlare_DOMAINS[$GLOBALS['MYHOST']])) idie($o.=" Error: site not found [".$GLOBALS['MYHOST']."]");
    $id=$ClaudFlare_DOMAINS[$GLOBALS['MYHOST']];

    $json='{"files":["'.$file.'"]}';

    $p=cufind("$id/purge_cache",'','DELETE',$json);
    $p=(array)$p['result']; $id=$p['id'];
    return $o=" DELETED: ".$id;
}
