...
function telegram_api_url() {
    ...
    if(!empty($GLOBALS['telegram_API_url'])) return $GLOBALS['telegram_API_url']."bot".$GLOBALS['telegram_API_key'].'/';
...
function telegram_edit($id,$channel,$s/*,$fotos=''*/) {
    ...
    if(!isset($e['edit_date']) && !strstr($e,'exactly the same as a current content')) idie("Telegram @".h($channel)." Edit error: ".nl2br(h(print_r($e,1))));
    return $id;
}
...
function ferror_log($s) {
    if(function_exists('idie')) { idie("TELEGRAM ERROR: ".nl2br(h($s))); }
    error_log($s."\n");
    exit;
}
