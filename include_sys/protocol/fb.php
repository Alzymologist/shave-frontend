...
function fb_delete($cf,$num) {
    $sm=ms("SELECT `id`,`i` FROM `socialmedias` WHERE `type`='post' AND `num`=".intval($num)." AND `net`='".e($cf['nett'])."'".ANDC()." LIMIT 1","_1",0);
    if($sm===false) return ret_ok('deleted from base');
    $x=fb_del($cf,$sm['id']); if($x['OK']!='OK') return $x; // ошибка
    msq("DELETE FROM `socialmedias` WHERE `i`=".intval($sm['i']) );
    $x['smi']=$sm['i'];
...
// idie(nl2br(h($s)));
	...
	test_file($file,"Foto not found: ".h($file));
	$fotos[]=$file;
	if(sizeof($imgs[0])==1) {
...
// idien($s);
    ...
    $sm=ms("SELECT `i`,`id`,`cap_sha1` FROM `socialmedias` WHERE `type`='post' AND `num`=".intval($p['num'])." AND `net`='".e($cf['nett'])."'".ANDC()." LIMIT 1","_1",0);
    if($sm!=false) { // Edit
	...
	msq_update('socialmedias',arae(array('cap_sha1'=>$cap_sha1)),"WHERE `i`=".intval($sm['i'])." LIMIT 1");
...
function fb_run($a,$user='',$pass='',$domain='',$args=array()) { $go=0;
    ...
    // где лежит скрипт
    $script_file=$GLOBALS['filehost']."extended/phantom-facebook.js"; test_file($script_file,"Error: not install ".$script_file);
