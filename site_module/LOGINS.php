...
WHERE `".$conf['mode']."` >= '".($conf['mode']=='timelast' ? date("c",$lastcom) : $lastcom)."' AND (`password`!='' OR `openid`!='') ORDER BY `".e($conf['mode'])."`"
." LIMIT 20","_a");

// WHERE `".e($conf['mode'])."`>='".e($conf['mode']=='timelast'?date("c",$lastcom):$lastcom)."' 
