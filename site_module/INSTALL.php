...
&& substr($_SERVER['QUERY_STRING'],0,4)!='win_'

) {
	$file=ltrim(urldecode($_SERVER['QUERY_STRING']),'/');
	$i=0; if($file[strlen($file)-1]=='|') { $i=1; $file=rtrim($file,'|'); }

	if($file != rpath($file)) idie("Error 404");
    ...
    $s=preg_replace(
    "/^<code>\s*<span style=\"color\: #000000\">\s*<span style=\"color\: #0000BB\">\&lt;\?php<br \/>/s",
    '<code><span style="color: #000000">',$s);
    $o=''; for($i=0;$i<strlen($s);$i++) { if(ord($s[$i]) >= 128) $o.=$s[$i]; }
    // $s=(iconv("utf-8",$GLOBALS['wwwcharset']."//TRANSLIT",$s));
    header("Content-Type: text/html; charset="
	.( ( strlen($o)/((int)substr_count($o,'Р')+0.1) ) < 11 ? 'UTF-8' : $GLOBALS['wwwcharset']));
    die($s);
} // на экран
...
setkey('Escape','',function(e){ if(idd('edit_text').value==idd('edit_text').defaultValue || confirm('exit no save?')) clean('editor'); },false);
...
<br><input type='submit' value='Go'></form>")."\")";
    ...
    function cfn($s) { return str_replace('!','',$s); }
    function cfm($s) { return str_replace('!',"<i class='e_button_accept'></i>",$s); }
...
*/
	...
	if(false==($p=ms("SELECT * FROM ".$GLOBALS['db_unic']." WHERE `id`=".intval($u),'_a',0))) return "salert('Unic ".$u." not found!',5000)";
	$p=$p[0];
...
Array
(
    [zi] => d05a2101058e0853803f0aeed3fcc321
    [names] =>  unic pass
    [unic] => 573741
    [pass] => 
    [mod] => INSTALL
    [a] => ch_users_
    [hashpage] => 1670700167-050f5831d136acfc
    [up] => 4-fb8082ef8295b05c977355b948014325
    [acn] => 0
    [ux] => 
)
*/
	...
	if(RE('swapmail')==1) { // поменять местами mail и mailw
	    msq_update($GLOBALS['db_unic'],arae(array('mail'=>$p['mailw'],'mailw'=>$p['mail'])),"WHERE `id`=".intval($u) );
	}
	...
	if($l!=$p['login']) { // изменить логин
	    ...
	    msq_update($GLOBALS['db_unic'],arae(array('login'=>$l)),"WHERE `id`=".intval($u) );
	    $p['login']=$l;
	}

	if(RE('cleanmailopenid')==1) { // сбросить openid
	    msq_update($GLOBALS['db_unic'],arae(array('openid'=>'')),"WHERE `id`=".intval($u) );
	}

	if(RE('cleanmail')==1) { // сбросить все емайл
	    msq_update($GLOBALS['db_unic'],arae(array('mail'=>'')),"WHERE `id`=".intval($u) );
	}

	if(RE('cleanmailw')==1) { // сбросить все емайлw
	    msq_update($GLOBALS['db_unic'],arae(array('mailw'=>'')),"WHERE `id`=".intval($u) );
	}

	if(RE('cleantel')==1) { // сбросить все tel
	    msq_update($GLOBALS['db_unic'],arae(array('tel'=>'','telw'=>'')),"WHERE `id`=".intval($u) );
	}

	$l=RE('newpassword'); if($l!='') { // изменить пароль
	    if(''==$p['login']) return "salert('Login is empty. Set the login!',5000)";
	    $i=md5($l.$GLOBALS['hashlogin']);
	    msq_update($GLOBALS['db_unic'],arae(array('password'=>$i)),"WHERE `id`=".intval($u) );
	}
    ...
    $o=
    "<div class='ll' onclick=\"majax('login.php',{a:'getinfo',unic:'".$u."'})\">open personal card</div>"
    ."<table border='0' cellspacing='10'><tr><td>"
	."<input type='hidden' name='editunic' value='1'>";

    if($p['openid']!='') $o.="<div class='nobr'><input name='cleanopenid' onclick=\"var i=idd('openidrem'); i.style.display=(i.style.display=='none'?'inline':'none')\" type=checkbox>"
	." Remove openid: <a id='openidrem' href='".h($p['openid'])."'>".h($p['openid'])."</a></div>";

    if($p['mail']!='') $o.="<div class='nobr'><input name='cleanmail' onclick=\"var i=idd('mailrem'); i.style.display=(i.style.display=='none'?'inline':'none')\" type=checkbox>"
	." Remove primary email: <a id='mailrem' href='mailto:".cfn(h($p['mail']))."'>".cfm(h($p['mail']))."</a></div>";

    if($p['mailw']!='') $o.="<div class='nobr'><input name='cleanmail' onclick=\"var i=idd('mailwrem'); i.style.display=(i.style.display=='none'?'inline':'none')\" type=checkbox>"
	." Remove secondary email: <a id='mailwrem' href='mailto:".cfn(h($p['mailw']))."'>".cfm(h($p['mailw']))."</a></div>";

    if($p['mail']!='' && $p['mailw']!='') $o.="<div class='nobr'><input name='swapmail' type='checkbox'> Swap emails</div>";

//    if($p['mail']!='' && $p['mailw']!='') $o.="<div>Swap email1 to email2: <input name='swapmail' onclick=\"var i=idd('mailrem'); i.style.display=(i.style.display=='none'?'inline':'none')\" type=checkbox> remove <a id='mailrem' href='mailto:".h(get_workmail($p))."'>".h(get_workmail($p))."</a></div>";
    if($p['tel']!='') $o.="<div class='nobr'><input name='cleantel' onclick=\"var i=idd('telrem'); i.style.display=(i.style.display=='none'?'inline':'none')\" type=checkbox>"
	."Remove primary tel: <b>".h($p['tel'])."</b></div>";

    if($p['telw']!='') $o.="<div class='nobr'><input name='cleantelw' onclick=\"var i=idd('telrem'); i.style.display=(i.style.display=='none'?'inline':'none')\" type=checkbox>"
	."Remove secondary tel: <b>".h($p['telw'])."</b></div>";

    $o.="<div class='nobr'><input onclick=\"var i=idd('newpassword'); i.style.display=(i.style.display=='none'?'inline':'none')\" type=checkbox>"
	." New password:  <input id='newpassword' style='display:none' name='newpassword' type=text size=10 value=''></div>";
    $o.="<div class='nobr'>New login: <input type=text size=10 name='newlogin' value='".h($p['login'])."'></div>";
...
if($a=='install') { // инсталляция
	...
	$serv=fileget_save($dir."servers.txt","https://lleo.me/blog Beta
https://lleo.me/dnevnik Stable
https://lleo.me Super Stable
https://binoniq.net Server Stable");

	$select_serv=fileget_save($dir."server.my","https://lleo.me/blog\n+basic");
...
// ЗЕНОНЗАЕБАЛ!!!!!11
	...
	$o=array(); foreach(explode("\n",$serv) as $l) { $l=trim($l,"\n\r\t "); if($l=='') continue;
		list($ser,$ver)=explode(' ',$l,2);
		$ser=str_replace(array('https://','http://'),'//',$ser);
		$o[$ser]=$ver.': '.$ser;
	}
...
if($a=='install_check') { // инсталляция - ЭТО ПРОИСХОДИТ ЕЩЕ НА СОБСТВЕННОМ СЕРВЕРЕ
	$pack=RE('pack'); $ser=RE('s'); // $ser=str_replace(array('http://','https://'),'//',$ser);
...
function msq_change_pole($table,$pole,$s){ if(msq_pole($table,$pole)!==false) msq("ALTER TABLE `".trim($table,'`')."` CHANGE `".$pole."` `".$pole."` ".$s); }
// добавить поле таблицы
function msq_add_pole($table,$pole,$s){ if(msq_pole($table,$pole)===false) msq("ALTER TABLE `".trim($table,'`')."` ADD `".$pole."` ".$s." NOT NULL"); }
// удалить поле из таблицы
function msq_del_pole($table,$pole){ if(msq_pole($table,$pole)!==false) msq("ALTER TABLE `".trim($table,'`')."` DROP `".$pole."`"); }
...
function msq_del_index($table,$pole){ if(msq_index($table,$pole)) msq("ALTER TABLE `".trim($table,'`')."` DROP INDEX `".$pole."`"); }
// создать таблицу
function msq_add_table($s){ msq($s); }
// удалить таблицу
function msq_del_table($table){ if(msq_table($table)) msq("DROP TABLE `".trim($table,'`')."`"); }
// удалить таблицу
function msq_truncate_table($table){ if(msq_table($table)) msq("TRUNCATE `".trim($table,'`')."`"); }

//======================================================================================
// похвастаться успешной установкой
function admin_pohvast() { return "<center><div id=soobshi><input type=button value='Похвастаться успешной установкой' onclick=\"document.getElementById('soobshi').innerHTML = '<img src=//lleo.me/blog/stat?link={httphost}>';\"></div></center>"; }
