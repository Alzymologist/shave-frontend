...
// idie("EE: g=$g admin=$admin podzamok=$podzamok");

if($GLOBLAS['ttl']) $GLOBLAS['ttl']=60*10;

// $GLOBALS['admin']=$GLOBALS['podzamok']=$GLOBALS['adm']=$GLOBALS['ADM']=0;

function CONTENTS($e) { global $ADM,$admin,$podzamok,$count;
...
if($ADM) $opt=array_merge($opt,array('invis_adm'=>"совсем скрытые заметки"));

// idie("ADMin=$admin adm=$ADM");

// if($GLOBALS['unic']==27658) {

// dier($GLOBALS);
/*
    [admin] => 1
    [accold] => vladimirberezin
    [acc] => vladimirberezin
    [ADM] => 
*/
...
Array (
    [] => по дате
    [rating] => по посещениям
    [invis] => подзамочные заметки
    [count] => счетчики посещений
)
*/
...
if($ADM && $g=='invis_adm') $o.=pr_zapisi(swhe(WHERE(andcc()." AND `Access`='admin'"))."ORDER BY z.`Date` DESC");
...
function andcc() { return ($GLOBALS['mnogouser']!=1?'1=1':"z.`acn`=".intval($GLOBALS['acn'])); }
