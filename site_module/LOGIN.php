...
$thispath=( strpos($GLOBALS['httphost'],$GLOBALS['MYHOST'])===false ? $GLOBALS['HTTPS'].'://'.$GLOBALS['MYHOST'].'/'.$GLOBALS['blogdir']
    :acc_link($GLOBALS['acc'],'/'.$GLOBALS['blogdir'])
);
...
if($a=='mailconfirm') { $js=$s=''; $u=intval($_GET['u']); $mail=$_GET['m'];
	...
	if(false===($a=ms("SELECT `mail`,`mailw` FROM $db_unic WHERE `id`=".intval($u),"_1",0)))  return "Попытка подтвердить email `".h($mail)."` при unic=`$u` не удалась!";
	...
	if($a['mail']==$mail) { msq_update($db_unic,array('mail'=>'!'.$mail),"WHERE `id`=".intval($u) ); $s.="<font color=green>Адрес `$mail` подтвержден как основной.</font>"; }
	if($a['mailw']==$mail) { msq_update($db_unic,array('mailw'=>'!'.$mail),"WHERE `id`=".intval($u) ); $s.="<font color=green>Адрес `$mail` подтвержден как дополнительный.</font>"; }
...
if(isset($_GET['getloginza'])) { // блядь все равно работать не будет
    // die(urlencode("https://lleo.me/dnevnik/login?getloginza=/api/widget?overlay=loginza&token_url=https://lleo.me/dnevnik/ajax/ulogin_xd.php?cache=0.0667119233358594&providers_set=openid,google,facebook,livejournal,vkontakte,odnoklassniki,twitter,yandex,mailru,myopenid,webmoney,linkedin,loginza,rambler,flickr,lastfm,verisign,aol,steam&ajax=true"));
    $s=uw(file_get_contents("http://loginza.ru".$_GET['getloginza']));
    $s=str_replace("</title>","</title><base href=\"https://loginza.ru/\">",$s);
//    file_put_contents($GLOBALS['filehost'].'LOGINZA.HTM',$s);
    die($s);

// <base href="https://www.example.com/">


//    http://loginza.ru/api/widget?overlay=loginza&token_url=https://lleo.me/dnevnik/ajax/ulogin_xd.php?cache=0.9053434646495874&providers_set=openid,google,facebook,livejournal,vkontakte,odnoklassniki,twitter,yandex,mailru,myopenid,webmoney,linkedin,loginza,rambler,flickr,lastfm,verisign,aol,steam&ajax=true
}
...
if(isset($_GET['loginza'])) {
    $s=file_get_contents("https://loginza.ru/api/authinfo?token=".$_GET['loginza']);
...
if(isset($_GET['ulogin'])) {
	$s=file_get_contents('https://ulogin.ru/token.php?token='.$_GET['ulogin'].'&host='.$_SERVER['HTTP_HOST']);
	include $GLOBALS['include_sys']."json.php"; $j=jsonDecode($s);
	$ll=login_do($j);
}
...
'Date'=>$modp,'Header'=>$modp,'Body'=>'@@@@@@@@@@@@222{_'.$modp.':_}@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@222',
...
*/
// $GLOBALS['_PAGE']['design']='ddddddddddddddddddddddddddd'; // modules($design);
...
$uf='https://ulogin.ru/img/photo.png'; // вот это не надо говно нам
	$img=( !empty($j['photo_big'])&&$j['photo_big']!=$uf ? $j['photo_big'] : (!empty($j['photo'])&&$j['photo'] != $uf ? $j['photo'] : '') );

	$site=( !empty($j['web']['default']) ? $j['web']['default'] : (!empty($j['web']['blog'])?$j['web']['blog']:'') );
...
if($p===false) { // НЕТ, ТАКОГО ЕЩЕ У НАС НЕ БЫЛО
	...
	msq_update($GLOBALS['db_unic'],arae($ara),"WHERE `id`=".intval($GLOBALS['unic']) );
if($conf['log']) logi('login_log.txt',"\np=false, UPDATE id=unic=`".$GLOBALS['unic']."`: openid=`".$openid."`: ".print_r($ara,1));
	...
	if(sizeof($ara)) msq_update($GLOBALS['db_unic'],arae($ara),"WHERE `id`=".intval($p['id']) );
...
if($a=='newpassword') {
    ...
    msq_update($GLOBALS['db_unic'],array('password'=>md5($np.$GLOBALS['hashlogin'])),"WHERE `id`=".intval($u) );
...
if($a=='forgotpas') {
	...
	false!==($p=ms("SELECT * FROM ".$GLOBALS['db_unic']." WHERE `id`=".intval(substr($log,1)),"_1",0))
    ) forgotpas_send(array($p));
...
/*** NFC ***/
function NFCHASH() {
	if($GLOBALS['mnogouser']) idie("Пока только для однопользовательского сайта, еще не дописал безопасное решение");
	$NFC=strtoupper(RE('NFC')); if($NFC=='') return '';
	if(preg_match("/[^0-9A-Z\:]/s",$NFC)) idie("Wrong symbols in NFC key: ".h($NFC));
	return md5($NFC.$GLOBALS['hashlogin']);
}

if($a=='logNFC') { // авторизация по ключу NFC
	$NFCHASH=NFCHASH(); if(empty($NFCHASH)) idie('Empty NFC');
	if(($id=ms("SELECT `id` FROM ".$GLOBALS['db_unic']." WHERE `NFC`='".e(NFCHASH())."'","_l",0))===false) idie('NFC key `'.h($NFC).'` not found!');
	if(getis_global($id)===false) idie('Error #1018');
	return reset_the_unic($GLOBALS['imgicourl'],$id);
}

if($a=='setNFC') { // авторизация по ключу NFC
	msq_update($GLOBALS['db_unic'],array('NFC'=>NFCHASH()),"WHERE `id`=".intval($GLOBALS['unic']) );
	return "salert('Saved!',600);";
}

if($a=='aboutNFC') {
    otprav("ohelpc('nfc_help','About login by NFC-card',\"".njsn("
<div style='max-width:500px;text-align:justify;' class='r'>
Cовременные телефоны умеют читать поднесенные к ним метки NFC. Некоторые браузеры
(например, Android Chrome) позволяют делать это прямо с сайта.<p>Ваш браузер - ")
."\"+('NDEFReader' in window ?\"<font color='green'>позволяет</font>\":\"<font color='red'>не&nbsp;позволяет</font>, дальше можно не читать\")+\"."
.njsn("<p>Меткой NFC может быть ключ от домофона или офиса, любая транспортная карта или банковская бесконтактная.
 Телефон может прочесть уникальный номер типа A0:CD:48:1E:DF чтобы использовать как ключ для логина.
<p>Бояться не надо: не существует способа испортить карту или получить доступ к
 деньгам и поездкам - и карты, и браузеры много поработали над политикой безопасности.
<p>Просто нажмите кнопку, разрешите телефону читать NFC и привяжите ключ к своему аккаунту,
чтобы потом легко логиниться им.
</div>")."\");");
}
...
function check_change_pass($u,$p,$h) {
	if(!$u||false==($pp=ms("SELECT * FROM ".$GLOBALS['db_unic']." WHERE `id`=".intval($u),"_1",0))) return "Error: unic #".$u;
