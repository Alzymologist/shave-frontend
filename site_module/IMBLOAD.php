...
$pp=ms("SELECT `opt`,`Date`,`Body`,`Header`,`DateUpdate`,`Access`,`num`,`DateDatetime` FROM `dnevnik_zapisi` ".WHERE("`DateDatetime`!=0 AND `DateUpdate`>".intval($time)." AND `DateUpdate`<".intval($timeto) )." ORDER BY `DateUpdate` DESC LIMIT ".$conf['limit'],"_a");
...
if($p['Comment_view']!='off' && strstr($conf['template'],'{comment}')) {
   $idzan=intval(ms("SELECT COUNT(*) FROM `dnevnik_comm` WHERE `DateID`="intval($p["num"]),'_l'));
