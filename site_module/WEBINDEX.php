...
function WEBINDEX_ajax() { $a=RE('a'); // запрос
	...
	$ipn=$q['IPN'];
	if(!ms("SELECT COUNT(*) FROM ".$GLOBALS['db_wi_ipn']." WHERE `ipn`=".intval($ipn),'_l',0)) msq_add($GLOBALS['db_wi_ipn'],arae(array('ipn'=>$ipn,'count'=>1)));
	else msq("UPDATE ".$GLOBALS['db_wi_ipn']." SET count=count+1 WHERE `ipn`=".intval($ipn) ); // увеличить счетчик

	// удалить старые
	msq("DELETE FROM ".$GLOBALS['db_wi_ajax']." WHERE `time`<".intval(time()-1000) );
	msq("DELETE FROM ".$GLOBALS['db_wi_link']." WHERE `time`<".intval(time()-1000) );
...
.($ipn==$IPN?' THE SAME':" (ipn: ".ipn2ip($ipn).")")
	...
	msq("UPDATE ".$GLOBALS['db_wi_ipd']." SET count=count+1 WHERE `i`=".intval($r['i']) ); // увеличить счетчик
...
function wi_getflag() { global $IPN,$memcache;
    ...
    if(!($p=ms("SELECT `IPN`,`net`,`BRO` FROM ".$GLOBALS['db_wi_ajax']." WHERE `ipn_user`=".intval($IPN),"_1",0))) return false;
    return $p;
}
...
function wi_delflag() { global $IPN,$memcache;
    ...
    msq("DELETE FROM ".$GLOBALS['db_wi_ajax']." WHERE `ipn_user`=".intval($IPN) );
    return;
}
...
return false; } // если такое было
    ...
    if(ms("SELECT COUNT(*) FROM ".$GLOBALS['db_wi_link']." WHERE `ipn`=".intval($IPN),"_l",0)) return true; // если с этии IPN же работаем
...
function wi_save($from,$to,$net,$country) {
    // если есть такой IP
    if(ms("SELECT COUNT(*) FROM ".$GLOBALS['db_wi_ip']." WHERE ".intval($from).">=`from` AND ".intval($to)."<=`to`","_l")) { $GLOBALS['msqe'].="error wi_ip base double"; return; }
    // есть ли такая сетка
