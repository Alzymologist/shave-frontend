...
function AUTOPOST_ajax() { ADMA(); $a=RE('a'); $nnet=RE0('nnet'); $num=RE0('num');
    ...
    if($a=='apost') {
    $link_id=ms("SELECT `id` FROM `socialmedias` WHERE `type`='post' AND `num`=".intval($num)." AND `net`='".e($cf['nett'])."'".ANDC(),'_l',0);
    include_once $GLOBALS['include_sys']."protocol/".preg_replace("/[^a-z]/si",'',$cf['net']).".php";
    if($link_id!=false) {
	...
	// exit;
	$p=ms("SELECT * FROM `dnevnik_zapisi` WHERE `Access`='all' AND `num`=".intval($num),"_1");
