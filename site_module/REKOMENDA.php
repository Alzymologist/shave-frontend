...
if($a=='edit') {
	$p=ms("SELECT `link`,`text` FROM ".$GLOBALS['db_rekomenda']." WHERE `n`=".intval($n),"_1",0);
...
if($a=='edit_') {
	...
	if($link!='') {
		msq_update($GLOBALS['db_rekomenda'],array('text'=>e($text),'link'=>e($link)),"WHERE `n`=".intval($n) );
		return "
...
if($a=='del') {
	msq("DELETE FROM ".$GLOBALS['db_rekomenda']." WHERE `n`=".intval($n) );
...
//---------------
	...
	if(!$pp) $GLOBALS['msqe']=$emsqe;
	else { $s=''; foreach($pp as $p) {
