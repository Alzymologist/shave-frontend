...
if($a=='omsg_read') { // сигнал "прочитано"
	...
	msq_update($db_fidomy,array('metka'=>'read'),"WHERE `point`='$fido_point' AND `id`=".RE00('id'));
	$arean=RE0('arean');
	$new=msgn_new($arean);
...
if($a=='allmsg_read') { // эха: пометить как прочитанные
	...
	ms("UPDATE $db_fidomy SET `metka`='read' WHERE `point`='$fido_point' AND `arean`=".intval($arean),"_l");
	return "
...
if($area=='netmail') { // вообще вынести мессагу с этим id
	...
	$n=ms("SELECT COUNT(*) FROM $db_fidomy WHERE `id`=".intval($id)." AND $points","_l",0);
	if(!$n) return "salert('NO NETMAIL #$id',500)"; // ничего
	if($n>2) return "salert('N=$n NETMAIL #$id',5000)"; // больше 2 участников переписки

	msq("DELETE FROM $db_fidomy WHERE `id`=".intval($id)." AND $points");
	if($n==1) msq("DELETE FROM $db_fido WHERE `id`=".intval($id)); // и если 1, то удалить и из общей базы
} else {
	if(0==ms("SELECT COUNT(*) FROM $db_fidomy WHERE `point`='$fido_point' AND `id`=".intval($id),"_l",0)) return "salert('NO: #$id',500)";
	msq("DELETE FROM $db_fidomy WHERE `point`='$fido_point' AND `id`=".intval($id));
}
...
if($a=='allmsg_new') { // эха: пометить как новые
	$arean=RE0('arean'); if(!$arean || !$fido_point) return '';
	ms("UPDATE $db_fidomy SET `metka`='new' WHERE `point`='$fido_point' AND `arean`=".intval($arean),"_l");
...
if($a=='allmsg_del') { // эха: удалить все
	$arean=RE0('arean'); if(!$arean || !$fido_point) return '';
	msq("DELETE FROM $db_fidomy WHERE `point`='$fido_point' AND `arean`=".intval($arean));
...
if($a=='allmsg_restore') { // эха: восстановить
	$arean=RE0('arean'); if(!$arean || !$fido_point) return '';
	$pp=ms("SELECT `id` FROM $db_fido WHERE `AREAN`=".intval($arean),"_a");
	if(!sizeof($pp)) return "salert('Сообщений в базе не найдено',2000)";
	msq("DELETE FROM $db_fidomy WHERE `point`='$fido_point' AND `arean`=".intval($arean)); // очистить
...
if($a=='change_podpiska') { // настройки
	...
	if(RE('i')==1) { // подписаться
		...
		msq("DELETE FROM $db_fidomy WHERE `point`='$fido_point' AND `arean`=".intval($arean)); // потереть флажки
	}
...
OR  `echo`='".e(strtr($l,'&','_'))."'","_a",0);
		...
		if($bb[0]['echonum']!=$b['echonum']) {
			$s.="<br>UPDATE $db_fido SET `AREAN` = '".$bb[0]['echonum']."' WHERE `AREAN`=".intval($b['echonum']);
			ms("UPDATE $db_fido SET `AREAN` = '".$bb[0]['echonum']."' WHERE `AREAN`=".intval($b['echonum']),"_l");
		}
		}
 }
...
if($a=='reply') { // Ответить на сообщение
	...
	$p=ms("SELECT `DATETIME`,`MSGID`,`BODY`,`SUBJ`,`FROMNAME`,`FROMADDR` FROM $db_fido WHERE `id`=".intval($id),"_1");
...
if($a=='delarea') { AD(); $arean=RE0('arean'); $area=arean2area($arean); // Убить эху
	...
	// 1. Убить у всех подписку
		msq("DELETE FROM $db_fidopodpiska WHERE `echonum`=".intval($arean));
	// 2. Убить все флажки у всех
		msq("DELETE FROM $db_fidomy WHERE `arean`=".intval($arean));
	// 3. Убить все мессаги в главной базе
		msq("DELETE FROM $db_fido WHERE `AREAN`=".intval($arean));
	// 4. Удалить имя из базы эх
		msq("DELETE FROM $db_fido_num WHERE `echonum`=".intval($arean));
...
//	if($GLOBALS['admin']) dier($_REQUEST);
	...
	if($fido_point && empty($id)) { // последний непрочитанный
		$id=ms("SELECT * FROM $db_fidomy WHERE `point`='$fido_point' AND `arean`=".intval($arean)." AND `metka`='new' ORDER BY `id` DESC LIMIT 1","_1");
		$id=$id['id'];
	}
...
".($id?"ebasa[mya].st=".ms("SELECT COUNT(*) FROM $db_fido WHERE `AREAN`=".intval($arean)." AND `id`>".intval($id),"_l").";":'')."
echotype();
...
function msgn_all($arean,$all=0) { global $fido_point,$db_fido,$db_fidomy;
	if(!$fido_point||$all) return ms("SELECT COUNT(*) FROM $db_fido WHERE `AREAN`=".intval($arean),"_l",0);

	if($GLOBALS['admin'] && 'netmail'==($area=arean2area($arean)) )
		return ms("SELECT COUNT(*) FROM $db_fidomy WHERE (`point`=".intval($fido_point)." OR `point`=1 OR `point`=0) AND `arean`=".intval($arean),"_l",0);

	return ms("SELECT COUNT(*) FROM $db_fidomy WHERE `point`='$fido_point' AND `arean`=".intval($arean),"_l",0);
}

function msgn_new($arean,$all=0) { global $fido_point,$db_fido,$db_fidomy;
if(!$fido_point||$all) return 0;
return ms("SELECT COUNT(*) FROM $db_fidomy WHERE `point`='$fido_point' AND `metka`='new' AND `arean`=".intval($arean),"_l",0);
}
...
$msq0="SELECT `DATETIME`,`BODY`,`id`,`SUBJ`,`FROMNAME`,`TONAME`,`FROMADDR`,`TOADDR` FROM $db_fido WHERE `AREAN`=".intval($arean)." ";
