...
$whe=array();
if(!$podzamok) $whe[]="(c.`scr`='0' OR c.`unic`=".intval($unic).")";
if(isset($_GET['unic'])) $whe[]="c.`unic`=".intval($_GET['unic']);
...
".WHERE(implode(' AND ',$whe))." ORDER BY c.`Time` DESC LIMIT ".intval($skip).",".intval($conf['rss_skip']));
