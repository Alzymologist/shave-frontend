...
//    file_put_contents('___epa.txt',print_r($_REQUEST,1));
 ...
 if($mode=='editevent') {
    ...
    msq_update('dnevnik_zapisi',arae($p),"WHERE `num`=".e($num).ANDC()); if($msqe) erflat('MySQL: '.$msqe);
    die("itemid\n".$num."\n"."url\n".getlink($p['Date'])."\n"."success\nOK\n");
 }
 }
}
...
if($a=='KES') {
    if(($s=ms("SELECT `Body` FROM `dnevnik_zapisi` WHERE `num`=".e($num).ANDC(),"_l"))===false) idie('Num not found'); // Такой заметки нет!
...
if($a=='rand') { $p=ms("SELECT `Date` FROM `dnevnik_zapisi`".WHERE().ANDC(),'_a1'); otprav("window.location='".getlink($p[rand(0,sizeof($p)-1)])."';"); }
...
if($a=='load') {
	if($num) { $x='num'; $v=intval($num); } else { $x='Date'; $v="'".e(RE('Date'))."'"; }
...
if($a=='ljpost') { ADMA();
	...
	if(($p=ms("SELECT `Date`,`Body`,`Header`,`opt`,`Access` FROM `dnevnik_zapisi` WHERE `num`=".e($num),"_1",0))===false)
...
$flat=(empty($admin_flat)?"http://www.livejournal.com/interface/flat":$admin_flat); // http://lj.rossia.org/interface/flat
	...
	if(($ljurl=ms("SELECT `url` FROM `socialmedia` WHERE `num`=".e($num)." AND `net`='lj'".ANDC(),"_l",0))!==false) {
...
//=================================== nocomment ===================================================================
if($a=='nocomment') { ADMA();
	if(($po=ms("SELECT `opt` FROM `dnevnik_zapisi` WHERE `num`=".e($num).ANDC(),"_1",0))===false) idie('false');
	...
	msq_update('dnevnik_zapisi',array('opt'=>e(ser($po))),"WHERE `num`=".e($num) );
...
ohelpc('alltags_".$idhelp."',\"Тэги заметки ".$num."\",\"".njsn($t)."\");
");
}
...
if($a=='loadhelp') { $name=RE('name'); ADMA();
	...
	include_once $include_sys."_modules.php";
	$s=modules($s);
	otprav("ohelpc('editor-help',\"Справка: редактор\",\"".njs($s)."\")");
}
//=================================== loadhelp ===================================================================
...
//	$txt=str_replace(array("\n",'"'),array('<br>','&quot;'),RE('txt'));
	$txt=str_replace(array("\n"),array('<br>'),RE('txt'));
	$body=ms("SELECT `Body` FROM `dnevnik_zapisi` WHERE `num`=".e($num).ANDC(),"_l",0);
...
//	file_put_contents('__oldbody.txt',$body);
	...
	msq_update('dnevnik_zapisi',array('Body'=>e($body)),"WHERE `num`=".e($num) );
...
otprav("
	LOADS([www_js+'tree.js',www_css+'tree.css'],function(){
	    ohelpc('foto','фотоальбом',\"".njs($s)."\");
	    tree('root');
	});
");
...
if($a=='savemove') { ADMA();
    ...
    if(intval(ms("SELECT COUNT(*) FROM `dnevnik_zapisi` WHERE `Date`='".e($New)."'".ANDC(),"_l",0))) idie("Заметка `".h($New)."` уже существует!");
...
$s="ohelpc('move',\"Перенос заметки ".h($p['Date'])."\",\"".njsn($s)."\");
...
//    [opt] => a:5:{s:12:"Comment_view";s:3:"off";s:7:"autokaw";s:2:"no";s:10:"autoformat";s:2:"no";s:8:"template";s:5:"blank";s:13:"Comment_media";s:3:"all";}
	...
	// if(($c=ms("SELECT `count` FROM `lleo`.`site_count` WHERE `lang`='".e($wwwhost.$file)."'",'_l',0))===false)

	$c=0; $fc=$filehost.dirname($file)."/.".basename($file).".count"; if(is_file($fc)) { $c=file($fc); $c=intval($c[0]); }

//	idie("count: ".$c." file: `".$fc."`");
...
//		'num'=>0,
	...
	if(is_file($fc)) rename($fc,$fc.'.del');
...
if($a=='editform') { ADX(); ADMA();
	if($num) $p=ms("SELECT * FROM `dnevnik_zapisi` WHERE `num`=".e($num).ANDC(),"_1",0);
...
if($a=='autopost_panel') { AD();
	$opt=unser(ms("SELECT `opt` FROM `dnevnik_zapisi` WHERE `num`=".e($num),"_l",0));
	$opt2=mkzopt($opt); ksort($opt2);
	foreach($opt as $n=>$l) unset($opt2[$n]);
	otprav("zabil('".$idhelp."_extautopost',\"<br>autopost: Under constructions<p>\");");
}
//----------- setting panel --------------
if($a=='settings_win') {
...
// idie("Ehhu!");
 ...
 ADMA(); //panel
	$aga=ms("SELECT `visible`,`opt` FROM `dnevnik_zapisi` WHERE `num`=".e($num).ANDC(),"_1",0);
...
$coms=array();
	...
	if($n=='template') { // выяснить о модулях
		$ainc=array();
		$inc=glob($GLOBALS['filehost']."template/*.html"); foreach($inc as $l) { $l=preg_replace("/^.*?\/([^\/]+)\.html$/si","$1",$l); $ainc[$l]=$l; }
		$inc=glob($GLOBALS['filehost'].accd()."template/*.html"); foreach($inc as $l) { $l=preg_replace("/^.*?\/([^\/]+)\.html$/si","$1",$l); $ainc[$l]=$l; }
...
if($a=='ch_dostup') { ADMA(); /*global $admincolors;*/ // смена доступа к заметке
	...
	foreach($admincolors as $n=>$l) { if($l[1]==$d) { $k=$admincolors[(++$n)%3];
		msq_update('dnevnik_zapisi',array('Access'=>$k[0]),"WHERE `num`=".e($num).ANDC());
...
if($a=='submit') { ADMA(); $e=explode(' ',trim(RE('names'))); if(isset($e['asave'])) unset($e['asave']);

	if($num) $p=ms("SELECT * FROM `dnevnik_zapisi` WHERE `num`=".e($num).ANDC(),"_1",0); else $p=array('opt'=>'');
...
if(count($_FILES)>0) {
	...
	$l=$f["name"][$a];
        test_file($f["tmp_name"][$a],"NOT FOUND: <hr><pre>".nl2br(h(print_r($f,1)))."</pre>");

        // найти расширение
        $ras=getras($l);

        $imgras=array('jpg','jpeg','gif','png','webp'); // разрешенные файлы картинок
	$mediaras=array('wav','mp3','ogg'); // разрешенные файлы медиа
        if(!in_array($ras,array_merge($imgras,$mediaras))) idie("*.".h($ras)." - unknown media type");
	...
	$fsave=$l; // новое имя
        $fsave=preg_replace("/[^0-9a-z\.\-\_]+/si",'-',$fsave); // оставили только латинские символы
        $fsave=preg_replace("/\.".$ras."$/si",'',$fsave); // убрали расширение
	if(in_array($ras,$imgras)) $ras='webp'; // переделывать все картинки в webp
	// найти незанятое имя
	$fotnum=0; while(file_exists(rpath($GLOBALS['filehost'].accd().$dt.'/'.( $fsave=($fotnum++).'.'.$ras )))) {}

	$df=rpath($GLOBALS['filehost'].accd().$dt.'/'.$fsave);
	testdir(dirname($df)); test_file(dirname($df),"Error dirname");
	$wf=$wwwhost.accd().$dt.'/'.$fsave;

	if(in_array($ras,$imgras)) {
	    // idie("Save file: [$df]");
	    obrajpeg($f["tmp_name"][$a],$df,$GLOBALS['foto_res_small'],$GLOBALS['foto_qality_small'],'');
	    $val=zameni_image('IMG',$pe,$i,$wf,$val); // 640,85
	} elseif(in_array($ras,$mediaras)) {
	    fileput($df,file_get_contents($f["tmp_name"][$a]));
	    $val=zameni_image('PLAY',$pe,$i,$wf." ".preg_replace("/\.(".implode('|',$mediaras).")$/si",'',$l),$val);
        }
	test_file($df,"Error save media");
}}
...
foreach($p as $n=>$l) if(isset($p1[$n])) { if($p1[$n]==$l) unset($p1[$n]); else $p[$n]=$p1[$n]; }
	...
	// если запись палюбому
	if(strlen($asave)>3 && !$num && !empty($p1['Date'])) { $num=ms("SELECT `num` FROM `dnevnik_zapisi` WHERE `Date`='".e($p1['Date'])."'".ANDC(),"_l",0); $p=$p1; }
	...
	if($num) {
	    ...
	    if(sizeof($p)) msq_update('dnevnik_zapisi',arae(array_merge($p,array('DateUpdate'=>time()))),"WHERE `num`=".e($num).ANDC()); if($msqe) idie($msqe);
...
//		if($num==0) { put_last_tmp($val); otprav(''); } else { del_last_tmp(); } // сохранять в tmp текст для новых
		$d=c($p1['Date']);
		if(empty($d)) {
		    if(strlen($asave)>3) otprav("var result_editor='NEW_ERROR DATE NULL';".$asave); // если свои скрипты
		    otprav("");
		}

		if(preg_match("/[^0-9a-z\-\_\.\/]+/si",$d) or empty($d) ) {
			$d=preg_replace("/[^0-9a-z\-\_\.\/]+/si",'',$d);
			if(strlen($asave)>3) otprav("var result_editor='NEW_ERROR DATE WRONG';".$asave); // если свои скрипты
		...
		if(0!=ms("SELECT COUNT(*) FROM `dnevnik_zapisi` WHERE `Date`='".e($d)."'".ANDC(),"_l",0)) {
			...
			if(strlen($asave)>3) otprav("var result_editor='NEW_ERROR DATE ".h($d.'_'.$r)."';".$asave); // если свои скрипты
		...
		msq_add('dnevnik_zapisi',arae($p1)); if($msqe) {
			if(strlen($asave)>3) otprav("var result_editor='NEW_ERROR MYSQL';".$asave); // если свои скрипты
			idie($msqe);
		}
		...
		if(strlen($asave)>3) otprav("var result_editor=\"NEW_OK ".$GLOBALS['num']." ".h($d)."\";".$asave);
		otprav_save("","window.top.location='".getlink($d)."';");
		//== новую заметку =====
	}

	if(strlen($asave)>3) otprav("var result_editor='EDIT_OK ".$num." ".h($p1['Date'])."';".$asave);
...
// file_put_contents("TEXT.txt",$s);
	...
	$l=str_replace(array('\0xA0','{nbsp}'),chr(0xA0),$l);
