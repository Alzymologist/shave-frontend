...
if(!empty($_GET['x'])) { $autorizatio=1; $ajax=0;
		include "../config.php";
		include $include_sys."_autorize.php";

	list(,$MYHOST)=explode('://',$httpsite); $x_domain=$xdomain.'.'.$MYHOST; // вычислили x.lleo.me
	if($_SERVER["HTTP_HOST"]!=$x_domain) die("error domain: ".$_SERVER["HTTP_HOST"]); // запретили загрузку с иных доменов
...
// die("upx=[$upx]");
	...
	if( !empty($_COOKIE['upx']) && ($upx=$_COOKIE['upx']) && upx_check($upx) && intval($unc=u2unic($upx)) ) { // если прошла проверку
...
var o='ajaxoff|CLOSE|id=xdomain|time=".time()."';
...
function upx_getis($unc) {
        if(($is=ms("SELECT * FROM ".$GLOBALS['db_unic']." WHERE `id`=".intval($unc),"_1",0))!==false) {
                $is=get_ISi($is);
