...
$s="<div style='margin-bottom:-16px;'><i><ul><li>"
."<font color=".('Error 404 - 1'==(@file_get_contents("http://home.lleo.me/dyndns/index.php?time=".time(),false,$stx))?'green':'red').">[home.lleo.me]</font>"
." <font color=".('Error 404 - 1'==(@file_get_contents("http://canada.home.lleo.me/dyndns/index.php?time=".time(),false,$stx))?'green':'red').">[canada.home.lleo.me]</font>"
."</li></ul></i></div>";
...
$price=parsehtm($s,'<span class="currency">US {s}lt;/span><span class="value">***</span>');
...
if($price2!==false && $price!==false && $price2!=$price) $rr="<div style='color:red'>{s}".h($price2)." <span class=br>".h($date2)."</span></div>";
// idie($s2);
...
$s=ms("SELECT `Body` FROM `dnevnik_zapisi` WHERE `num`=".intval($num),'_l',0);
if(!preg_match("/\n"
."\s*([^\|\n]+)\s*\|" //0 m1
."\s*([^\|\n]+)\s*\|" //1 m2
."\s*".$i."\s*\|" //2 m3
...
$z=preg_replace("/<\/a>.+$/s",'',$m[4])."</a><div style='font-size:20pt'>{s}".h($price)
...
msq_update('dnevnik_zapisi',arae(array('Body'=>$s)),"WHERE `num`=".intval($num) );
