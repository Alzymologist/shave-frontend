...
if($a=='unsubscribe' || $a=='ban_immediately') { $ajax=0; $mail=RE('mail'); // мгновенно отписаться или еще и забанить
...
function can_i_edit($p) { // разрешено ли редактировать?
    ...
    if(ms("SELECT COUNT(*) FROM `dnevnik_comm` WHERE `Parent`=".e($p['id']),"_l",0)) idie("Редактировать нельзя - уже есть ответы.");
...
otprav("ohelpc('whyban','Почему этот комментарий забанен?',\"".njs($o)."\")");
}
...
."</div>"; }
    ...
    otprav("ohelpc('my_ueban','мои забаненные',\"".njs($o)."\")");
}
//========================================================================================================================
...
mudsubmit=function(){
var x=".$click."-mucli;
if(x>0) { plays(www_design+'sound/bbm_tone.mp3'); return salert('Неправильно!<br>Сосредоточьтесь, у вас осталось '+x+' попыток!',1000); }
var x=muD-Math.floor(new Date().getTime()/1000);
if(x>0) { plays(www_design+'sound/bbm_tone.mp3'); return salert('Неправильно!<br>Будьте внимательны, у вас осталось '+x+' секунд!',1000); }
salert('Вы сделали много ошибок, но тест прошли!',1000);
capchaspec=0;
...
ohelpc('mucapcha','докажите, что вы не робот',\"".njsn("<table border=0 cellspacing=5 width=300>"
."<tr><td colspan=3 style='background-color:#4990E2; color:white; text-weight:bold; font-size:13px; padding:20px;'>Выберите все изображения,<br>на которых присутствуют<div style='font-size:38px'>мудаки</div></td></tr>"
...
."<tr><td colspan=3><input type='button' value='ГОТОВО' onclick='mudsubmit()'></tr></table>")."\");
...
//    idie(nl2br(h(fileget($GLOBALS["host_log"]."comment_foto_banned.log"))));
    ...
    otprav("salert('Не, извините, только админ это может',2000);");
...
/*
idie_error("Ставить оценки могут только залогиненные.
...
<p><center><input type='button' value='залогиниться' onclick=\"majax('login.php',{a:'do_login'})\"></center>");
*/

idie_error("Вы впервые на этом сайте?

Тогда просто еще раз обновите страницу, сайт вас запомнит,
и отныне вы сможете делать здесь всё, что захотите.

Если вы уже обновили страницу и все равно появилась эта ошибка -
значит, у вас в браузере отключены куки, скрипты или установлены
какие-то блокираторы.");
...
if($a=='paren2') { if(!$id) otprav(''); // показать коммент
	$parent=ms("SELECT `parent` FROM `dnevnik_comm` WHERE `id`=".e($id),"_l",0); // достанем parent
        $p=ms("SELECT * FROM `dnevnik_comm` WHERE `id`=".e($parent),"_1",0);
        $opt=ms("SELECT `opt` FROM `dnevnik_zapisi` WHERE `num`=".e($p['DateID']),"_1"); $GLOBALS['opt']=mkzopt($opt);
...
//========================================================================================================================
if($a=='otprav_comment') { otprav_comment(ms("SELECT * FROM `dnevnik_comm` WHERE `id`=".e($id),"_1",0)); } // просто отправить коммент
...
//	if(!$unic) otprav("А у вас всегда куки отключены?");
	...
	if($IS['loginlevel']<3) idie_error("Ставить оценки могут только залогиненные.
Залогиненным считается тот, кто залогинился соцсетью,
либо - придумал и вписал в личной карточке логин/пароль,
а затем подтвердил email, получив письмо и пройдя по ссылке.
<p><center><input type='button' value='залогиниться' onclick=\"majax('login.php',{a:'do_login'})\"></center>");
...
*/
	...
	if($a==ms("SELECT `var` FROM `dnevnik_plusiki` WHERE `commentID`=".e($id)." AND `unic`=".e($unic),'_l',0)) ktogadilminusom($id,'double'); // сам свои
...
otprav("ohelpc('ktominusil','кто ставил плюс/минус',\""
...
if($a=='editsend') { // прислан отредактированный комментарий
	...
	if($text=='') $a='del'; // если прислан пустой коммент - инициировать его удаление
	else { // а иначе отработать редактирование
		...
		if($text==$p['Text']) {
	        ...
	        $ara=$p; $ic=$GLOBALS['include_sys']."onComment.php"; if(is_file($ic)) include_once $ic; // передает $ara и $p
			otprav("clean('$idhelp');"); // если текст не изменился - просто закрыть
		}
...
/*
    setkey('Escape','',function(e){ if( (e.target && e.target.value.length<2)||confirm('Закрыть комментарий без сохранения?')) clean('{idhelp}'); },false,1);
*/

comnum++; ohelpc('".$idhelp."',\"".($admin?h($p['Name']):"редактирование")."\",\"<div id='commentform_".$comnu."'>"
...
$img=openimg($FILE["tmp_name"],$itype);
        if($img===false) idie(LL('Comments:foto:musor',implode(', ',$GLOBALS['foto_rash']))); // шо за мусор?
...
if($IS['capchakarma']!=1) {
    ...
    if($IS['capchakarma']>=60 && $IS['capchakarma']<100) { // спецкапча
	$mu=ms("SELECT `timelast` FROM `unictemp` WHERE `unic`=".intval($GLOBALS['unic']),"_l",0);
	if(time() < $mu+($IS['capchakarma']-59)*10 ) otprav("salert('Ошибка! Вы должны пройти тест заново!',1000);majax('comment.php',{a:'mucapcha',e:'start'})");
    } else {
...
<input type=hidden name='capcha_hash' value='".antibot_make($karma)."'></td><td>".antibot_img()."</td></tr></table>")."\");");
	if($IS['capchakarma']==0) $ara_kartochka['capchakarma']=(isset($GLOBALS['capchafirst'])?$GLOBALS['capchafirst']:1); // пометить в базе, что капчу однажды ввел
    }
}
// ============ // если нужна проверка капчи ==============
...
if($id && !isset($GLOBALS['ara_nomail'])) { $p=get_user_toans($ara,$id); if($ara['unic']!=$p['id']) { // если это ответ (не в корне коммент), остальное проверит сама процедура
...
// if($unic==4) { msq("UPDATE ".$GLOBALS['db_unic']." SET teddyid=0 WHERE id=4 LIMIT 1"); dier($p); }
    ...
    if(!empty($GLOBALS['tg_bot']) && $p['telegram']  // если был установлен бот telegram
    && ($p['opt']['tgcom'] || $p['opt']['tgcom1'] && $p['group']==1) // и разрешены оповещения при комментах всех или админа
    ) { // если был telegram и разрешены оповещения
    $ttxt="".$ara['Name']." отвечает вам в заметке ".getlink($p['Date'])
."\n".$p['Date']." - ".$p['Header']
."\n\n".$ara['Text'];
...
// ===============================================================
    ...
    $ic=$GLOBALS['include_sys']."onComment.php"; if(is_file($ic)) include_once $ic; // передает $ara и $p
...
// idie("Комментарии в этом дневнике в ближайшие дни отключены, ведутся работы. Приношу извинения за неудобства.");

 razreshi_comm();

if($dat==0) $dat=ms("SELECT `DateID` FROM `dnevnik_comm` WHERE `id`=".e($id),"_l",0); if($dat===false) idie("Фатальный сбой.");

$capchakarma=$IS['capchakarma'];
if($capchakarma>=60 && $capchakarma<100) { $capchakarma=1; $capchaspec=1; } else $capchaspec=0; // спецдиапазон мудаков
...
function del_comm($id,$l=1,$delcache=1) { $id=intval($id); if(!$id) return " alert('id=0?!');";
	...
	if($l and ms("SELECT COUNT(*) FROM `dnevnik_comm` WHERE `Parent`=".e($id),"_l",0) ) { // если у него есть потомки - просто пометить
	...
	$Parent=intval(ms("SELECT `Parent` FROM `dnevnik_comm` WHERE `id`=".e($id),"_l",0)); // сперва запомнили верхний
	$unic=intval(ms("SELECT `unic` FROM `dnevnik_comm` WHERE `id`=".e($id),"_l",0)); // узнать unic
	...
	if( ! $Parent // если он был в корне
		or ms("SELECT `Time` FROM `dnevnik_comm` WHERE `id`=".e($Parent),"_l",0) // или его верхний не удален
		or ms("SELECT COUNT(*) FROM `dnevnik_comm` WHERE `Parent`=".e($Parent),"_l",0) // если у верхнего есть другие потомки
