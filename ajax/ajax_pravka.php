...
$id=RE0('id'); $a=RE('action').RE('a'); ADH(); ADMA(1);
	...
	if($a=='del') { if($GLOBALS['ADM']) msq_del($GLOBALS['db_pravka'],array('id'=>$id)); pravka_otvet_e(); } // удалить говно из базы
...
/*
	setkey('Escape','',function(a,b){clean('opechatku')},true,1);
...
function pravka_basa_replace($data,$txt) { if(!$GLOBALS['ADM']) return; // от кулхацкеров
...
function pravka_basa_add($data,$stdprav,$metka='new') { // добавить сигнал в базу (безопасное)
	if($GLOBALS['ADM']) pravka_basa_add1($data,$stdprav,$metka);
}

function pravka_basa_add1($data,$stdprav,$metka='new') { // добавить сигнал в базу (опасное!)
	msq_add($GLOBALS['db_pravka'],arae(array(
	    'stdprav' => $stdprav,
	    'Date' => $data,
	    'unic' => $GLOBALS['unic'],
	    'acn' => RE0('acn'),
	    'text' => $GLOBALS['text'],
	    'textnew' => $GLOBALS['textnew'],
	    'metka' => $metka
	)));
}

function pravka_basa_p($id) {
	$p=ms("SELECT * FROM `".$GLOBALS['db_pravka']."` WHERE `id`=".intval($id),'_1',0);
...
function pravka_basa_metka($id,$metka,$answer) {
    if(!$GLOBALS['ADM']) return;
    if(isset($GLOBALS['rdonly'])) return;
    msq_update($GLOBALS['db_pravka'],arae(array('metka'=>$metka,'Answer'=>$answer)),"WHERE `id`=".intval($id) );
}
...
function pravka_priem($data,$text,$textnew) { // global $_RESULT; // прием опечаток от населения!
	...
	if($nzamen == 0) {
		...
		if(preg_match("/\&[a-z0-9\#]+\;/si",$oldtxt,$m)) pravka_otvet("Ошибка, словосочетание не найдено.\n\nАдмин! А попробуй-ка убрать из исходника значки типа '".h($m[0])."'
...
function pravka_otvet($s) { idie(nl2br($s)); }
