...
if(isset($realdom) && $realdom==$acc) $acc='';
...
if(!empty($acc)) {
  ...
  if(false==strpos($acc,'.')) { // поддомен пользователя
	...
	else { $acn=$p['acn']; $odomain=$p['domain']; $otime=$p['time']; $ounics=$p['unic']; $ADM=is_unics($unic,$ounics); if($ADM) $ttl=0; }
  } else { // заход как бы с внешнего домена, но в поддомен пользователя
	if(false==($p=ms("SELECT `acn`,`acc`,`unic`,`time` FROM `jur` WHERE `domain`='".e($acc)."'","_1"))) { $acn=-1; $ADM=$otime=0; $ounics=$odomain=''; }
...
НЕТУ БОЛЬШЕ АПИ ФЕЙСБУКА, ЗАКРЫЛ ОН ЕГО БЛЯДЬ ЕБАНАЯ
...
idie("<html>
<head><script type='text/javascript' language='JavaScript' src='".$GLOBALS['wwwhost']."js/main.js'></script></head>
...
if($ii===false && false===
    ($ii=ms("SELECT * FROM `socialmedias` WHERE `net`='".e($nett)."' AND `num`=".intval($num)." AND `comm`=".intval($comm).ANDC()." LIMIT 1","_l",0))
) idie("Error 507");

// idie('OK');

    if(($l=call_user_func($fn,$ii,$one))=='OK') otprav("clean('autoposte_".$ii."');");
...
if($a=='list') { ADMA(); // `type`='post' AND
    if(!sizeof($r=ms("SELECT `i`,`type`,`net`,`comm`,`id` FROM `socialmedias` WHERE `num`=".intval($num).ANDC(),"_a",0))) otprav("salert('empty',200);");
    $js="var s='';";
...
?}"
."{?ifdel:
1:<i class='l e_remove' title='delete post {#url}' onclick=\\\"if(confirm('Delete?')) majax('protocol.php',{a:'del',i:'{#i}',nett:'{nett}',num:'{#num}',comm:'{#comm}',n:'{#n}'".addtgs()."})\\\"></i>
*:
?}"
...
$s=''; foreach($r as $n=>$p) { list($net,$user)=explode(':',$p['net']);
    ...
    $url=call_user_func($fn,$p['id'],$user,$p['type']); // .($p['comm']==0?'':'#'.$p['comm'])
...
//    'postmode'=>RE('postmode'),
    ...
    'num'=>$num,'nett'=>$p['net'],'net'=>$net,'n'=>$n,'link'=>$link,'comm'=>$p['comm'],'user'=>$user,'url'=>$p['id']
// ,'led'=>('ledyellow.png'//	!function_exists($net.'_screen')?'ledyellow.png':(call_user_func($net.'_screen')==0?'ledgreen.png':'ledred.png')
//    )
));
}
...
$r=ms("SELECT `i`,`type`,`net`,`id`,`url` FROM `socialmedias` WHERE `net`='".e($nett)."' AND `num`=".intval($num).ANDC(),"_a",0);
...
$t="s=s+\"<div id='autopostr_{n}'><img src='".$www_design."img/ajaxm.gif'> {net} <b>{user}</b></div>\";
apostn['{n}']=1; setTimeout(\"majax('protocol.php',{a:'post',num:'{num}',comm:'{comm}',n:'{n}'".addtgs()."});\",50);";

foreach(explode("\n",load_autopost()) as $a=>$l) { if(($l=c0($l))=='') continue;
    $r=explode(' ',$l,4);
    if('#'==substr($r[0],0,1)) continue; // закомментированные #строки пропускать
    $js.=mpers($t,array('num'=>$num,'comm'=>$comm,'n'=>$a,'net'=>$r[0],'template'=>$r[1],'user'=>$r[2],'data'=>$r[3]));
}
...
if($comm) {
    ...
    if(($p=ms("SELECT z.Header,z.Access,c.Text,c.unic,c.Time,c.Name,c.Parent,c.whois,c.scr FROM `dnevnik_zapisi` AS z, `dnevnik_comm` AS c
 WHERE z.`num`=".intval($num)." AND c.`id`=".intval($comm),"_1",0))===false) idie(LL('ljpost:notfound')); // Такой заметки нет!
    if($p['scr'] /*|| $p['Access']!='all'*/) idie('comment screen');

    $datas=array_merge($p,array('num'=>$num,'comm'=>$comm,'n'=>$n,'net'=>$net,'template'=>$r[1],'user'=>$r[2],'password'=>$r[3],
'Header'=>h($p['Header']),'Date'=>$p['Date'],'text'=>h($p['Text'])
	),$r); unset($datas['Text']);

} else {

    if(($p=ms("SELECT `Date`,`Header`,`Body`,`opt`,`Access`,`visible` FROM `dnevnik_zapisi` WHERE `num`=".intval($num).ANDC(),"_1",0))===false) idie(LL('ljpost:notfound')); // Такой заметки нет!
//    if($p['Access']=='admin'||$p['visible']!=1) idie(LL('twitter:notpost')); // админские и невидимые заметки не постим

    // принудительно менять адреса родного сайта на https
    if(isset($GLOBALS['HTTPS']) && $GLOBALS['HTTPS']!='https') {
	$GLOBALS['HTTPS']='https';
	$GLOBALS['httpsite']=str_replace('http://','https://',$GLOBALS['httpsite']);
	$GLOBALS['httphost']=str_replace('http://','https://',$GLOBALS['httphost']);
    }
...
if(preg_match("/\{text(\d+)\}/si",$t,$m)) { // lj-cut
    ...
    $datas=array_merge($r,array('num'=>$num,'comm'=>$comm,'n'=>$n,'net'=>$net,'template'=>$r[1],'user'=>$r[2],'password'=>$r[3],
'url'=>getlink($p['Date']),
'urls'=>str_replace('http://','https://',getlink($p['Date'])),
'Header'=>$p['Header'],'Date'=>$p['Date'],
	'templ'=>$t,
	'text'=>$s,
	'txt'=>$txt
	));
...
if(!$comm && $p['Access']!='all' && (!isset($r['conf']['all']) || $r['conf']['all']!='allow')) $s="<b>$nett</b> error: page access not 'All' (".RE('postmode').")";
