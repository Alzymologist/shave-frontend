...
"`DateDate`>'".(time()-$day*86400)."'
 AND NOT EXISTS (SELECT `url` FROM `dnevnik_posetil` AS p WHERE `unic`=".intval($unic)." AND d.`num`=p.`url`)"
)
." ORDER BY `Date` DESC");
...
$o=implode('<br>',$m);
otprav("ohelpc('notseen',\"мои непрочитанные заметки\",\"".njs($o)."<hr width=30%>".$dop."\");");
}
...
if($a=='search') {
    ...
    $se_unic=RE0('search_unic');
    $se_name=c(RE('search_name'));
...
.selecto('oknom_search_type',RE("type"),array('zapisi'=>'в заметках','comm'=>'в комментариях','name'=>'в именах'),"name='type' onchange='changesearch(this.value)'")
."<div id='search_no' style='text-align:left;display:".(RE("type")=='comm'?'block':'none')."'>"
."<div>автор: <INPUT id='oknom_search_name' TYPE='text' name='search_name' value=\"".h($se_name)."\" SIZE='40' MAXLENGTH='40'></div>"
."<div>если известен unic автора: <INPUT id='oknom_search_unic' TYPE='text' name='search_unic' value=\"".($se_unic?$se_unic:'')."\" SIZE='20' MAXLENGTH='10'></div>"
."</div>"
." <input type='submit' value='Search'></center></form><p>";

$jscripts="changesearch=function(e){ idd('search_no').style.display=(e=='comm'?'block':'none'); };";
...
if($type=='zapisi') {
    $nlim=10;
    $SEAR="FROM `dnevnik_zapisi` ".WHERE("(`Body` LIKE '%".e($se)."%' OR `Header` LIKE '%".e($se)."%'".(intval($se)===$se?" OR `num`=".e($se):'').")".ANDC());
	...
	if(!function_exists('modules')) include_once $include_sys."_onetext.php";
...
// dier($p['Body']);
		...
		$GLOBALS['article']=$p; $p['Body']=modules($p['Body']);

		// пока не работает добивка до 10, да и нахуй оно надо
/*
	     $i=stristr($p['Body'],$se); // if(!$i) continue;
...
//	dier($p);
	    ...
	    foreach($o as $in=>$i) { if($in>10) { $t[]="<i><b>...и ещё подобных совпадений ".(sizeof($o)-10)."</b></i>"; break; }
...
// idie("comm: $se (unic=$se_unic name=$se_name)");

$SEAR="FROM `dnevnik_comm` AS c LEFT JOIN ".$GLOBALS['db_unic']." AS u ON c.`unic`=u.`id` WHERE ";

if($se_unic) $SEAR.="u.`id`=".intval($se_unic)." AND c.`Text` LIKE '%".e($se)."%'";
elseif($se_name!='') $SEAR.="c.`Name` LIKE '%".e($se_name)."%' AND c.`Text` LIKE '%".e($se)."%'";
else $SEAR.="(c.`Text` LIKE '%".e($se)."%'".(1*$se==$se?" OR c.`id`=".intval($se):'').")";

if($podzamok) $SEAR.=" AND c.`scr`='0'";

$nlim=10;

	$GLOBALS['n_real']=$n=intval(ms("SELECT COUNT(*) ".$SEAR,"_l"));

	// потом разобраться, может в комменты добавлять acn
	$s.=($n? pr_comments_("SELECT c.`id`,c.`unic`,c.`group`,c.`Name`,c.`Text`,c.`Parent`,c.`Time`,c.`whois`,c.`rul`,c.`ans`,
c.`golos_plu`,c.`golos_min`,c.`scr`,c.`DateID`,c.`BRO`,
u.`capchakarma`,u.`mail`,u.`admin`,u.`openid`,u.`realname`,u.`login`,u.`img` ".$SEAR." ORDER BY `Time` DESC LIMIT $nskip,$nlim"):"<center>ничего не найдено</center>");

// c.`unic`=".$id.($podzamok?'':" AND c.`scr`='0'")." ORDER BY `Time` DESC LIMIT $nskip,".($nlim)
...
WHERE c.`unic`=".intval($id).($podzamok?'':" AND c.`scr`='0'")."
...
if($a=='dostup') { // смена доступа
        ...
        $u=RE0('unic');
        $v=RE('value');
 	ms("UPDATE ".$GLOBALS['db_unic']." SET `admin`='".e($v)."' WHERE `id`=".intval($u),"_l",0);
	$p=ms("SELECT * FROM $db_unic WHERE id=".intval($u),"_1",0); $s=pr_unics0($p);
	otprav("
...
if($admin) {
$p['N']=ms("SELECT COUNT(*) FROM `dnevnik_posetil` WHERE `unic`=".intval($p['id']),"_l"); if($p['N']==0) $p['N']=' ';
}
...
$search=RE('search');
	...
	if($search!='') {
		...
		$search_unic=RE0('search_unic');
		if(($search_name=RE('search_name'))!='') $ar.=",search_name:'".h($search_name)."'";
		if(($search_unic=RE0('search_unic'))) $ar.=",search_unic:".intval($search_unic);
...
$n=(isset($GLOBALS['n_real'])?$GLOBALS['n_real']:RE0('n'));

// idie("n=$n nskip=$nskip nlim=$nlim"); n=18 nskip=0 nlim=10


if($nskip!=0) {
	$m="majax('okno.php',{".$ar.",n:'$n',nskip:'".($nskip-$nlim)."'})";
	$prev="<span class=l onclick=\"$m\">&lt;&lt; предыдущие ".($nlim)."</span>";
	$setkey.='setkey("ArrowLeft","ctrl",function(e){'.$m.'},true);';
	$setkey.='setkey("Digit4","",function(e){'.$m.'},true);';
} else $prev='';

if($nskip<($n-$nlim)) {
	$m="majax('okno.php',{".$ar.",n:'$n',nskip:'".($nskip+$nlim)."'})";
	$next="<span class=l onclick=\"$m\">следующие ".($nlim)." &gt;&gt;</span>";
	$setkey.='setkey("ArrowRight","ctrl",function(e){'.$m.'},true);';
	$setkey.='setkey("Digit7","",function(e){'.$m.'},true);';
} else $next='';

$prevnext="<p>".mk_prevnest($prev,$next);

// idie("prevnext=[".h($prevnext)."]"); //  # prev: $prev next: $next nlim: $nlim nskip: $nskip n: $n");

otprav("ohelpc('okno_$a',\"".njs($legend)."\",\"".njs($prevnext.$s.$prevnext)."\");
...
function pr_comments_($sql) { $s=''; $pp=ms($sql,"_a"); $colnewcom=sizeof($pp); if(!$colnewcom) return $s;
        $s.="<b>Комментариев: ".$colnewcom.(($n==RE0('n'))?" из ".$n:'')."</b>";
        $tmpDate='';
	...
	if(!isset($p['imgicourl'])) $p=get_ISi($p);
        ...
        if($tmpDate!=$d) {
		$x=ms("SELECT `Date`,`Header` FROM `dnevnik_zapisi` WHERE `num`=".intval($d),"_1");
...
function get_arenum0($n) { global $areans; if(!isset($areans)) $areans=array();
        ...
        $s=ms("SELECT `echo` FROM ".$GLOBALS['db_fido_num']." WHERE `echonum`=".intval($n),"_l");
