...
<channel>
  <title>".h($admin_name).": блог</title>
  <lastBuildDate>";

$s="</lastBuildDate>
  <link>".h($httphost)."</link>
  <description>".h($admin_name).": блог</description>
  <generator>Binoniq 3.0</generator>
  <wfw:commentRss>".h($httphost)."rssc"."</wfw:commentRss>
  <ya:more>".h($httpsite.$mypage)."?skip=".($skip+$RSSZ_skip)."</ya:more>
  <image>
    <url>".h($httphost)."design/userpick.jpg"."</url>
    <width>120</width>
    <height>155</height>
  </image>
";
...
$s .= "\n<item>
	...
	<author>".h($httphost)."</author>
	<pubDate>".date("r", $p["DateUpdate"])."</pubDate>
	<link>".h($link)."</link>
	<description>".str_replace(array('&','<','>'),array('&amp;','&lt;','&gt;'),$Body)."</description>
	<title>".h($Header)."</title>
	<comments>".h($link)."</comments>
</item>\n";
}
