...
$days=RE00('days'); if(!$days) $days=1;

$t=(time()-86400*$days);

// блять, а если две базы использует? $db_unic_bases=\"dnev lleoaharu\";
if(isset($GLOBALS['db_unic_bases'])) {

// return "FOR3";
// $WER_DEL_OLD="1=1";
$WER_DEL_OLD="`openid`='' AND `password`='' AND `opt`='' AND `capchakarma`<2 AND `mail` NOT LIKE '!%' AND `tel` NOT LIKE '!%'";

    foreach(explode(' ',$GLOBALS['db_unic_bases']) as $l) if(c($l)!='') {
...
AND `id` NOT IN (SELECT `unic` FROM `$l`.`golosovanie_golosa`)
AND `id` NOT IN (SELECT `unicfrom` FROM `$l`.`mailbox`)
AND `id` NOT IN (SELECT `unicto` FROM `$l`.`mailbox`)
";
...
// $unic_dnevnik_posetil=ms("SELECT `unic` FROM `dnevnik_posetil` WHERE `date`>$t","_a1",600);
$ttl600=60;
//$unic_dnevnik_comm=ms("SELECT `unic` FROM `dnevnik_comm`","_a1",$ttl600);
$unic_dnevnik_plusiki=ms("SELECT `unic` FROM `dnevnik_plusiki`","_a1",$ttl600);
$unic_golosovanie_golosa=ms("SELECT `unic` FROM `golosovanie_golosa`","_a1",$ttl600);
$unic_mailfrom=ms("SELECT `unicfrom` FROM `mailbox`","_a1",$ttl600);
$unic_mailto=ms("SELECT `unicto` FROM `mailbox`","_a1",$ttl600);

return nl2br("FOR [$days]
unic_dnevnik_comm      = ".sizeof($unic_dnevnik_comm      )."
unic_dnevnik_plusiki   = ".sizeof($unic_dnevnik_plusiki   )."
unic_golosovanie_golosa= ".sizeof($unic_golosovanie_golosa)."
unic_mailfrom          = ".sizeof($unic_mailfrom          )."
unic_mailto            = ".sizeof($unic_mailto            )."
");

dier($unic_dnevnik_posetil,sizeof($unic_dnevnik_posetil)." ".$GLOBALS['msqe']);
...
AND `id` NOT IN (SELECT `unic` FROM `golosovanie_golosa`)
AND `id` NOT IN (SELECT `unicfrom` FROM `mailbox`)
AND `id` NOT IN (SELECT `unicto` FROM `mailbox`)
";
...
dier("DELETE FROM `unic` WHERE $WER_DEL_OLD");
...
//----------------
$WER="WHERE `date`<".($time-86400*$days2)." LIMIT 1000";
...
//                usleep(1000);
	...
	foreach($r as $n=>$l) {
		msq("UPDATE `dnevnik_zapisi` SET view_counter=view_counter+$l WHERE `num`=".intval($n) ); if(!empty($GLOBALS['msqe'])) break;
        }
...
."</div></div>"
.($GLOBALS['msqe']==''?'':"<div>".njsn($GLOBALS['msqe'])."</div>")
."</fieldset>\");
