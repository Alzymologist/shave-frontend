...
// function smatch($e,$s){ return (preg_match($e,$s,$m)?$m[1]:false); }
...
//	foreach($r as $l) { if(false===($engine[$l]=smatch("/".$l."\=([^\s\$]*)/s",$q))) $a['CONFIG:'.$l]="Var `".$l."` not set"; }
	...
	$a['mbstring']=(function_exists('mb_convert_encoding')?"OK":"install mbstring module: sudo apt install php".('7'==substr(phpversion(),0,1)?'7.0':'')."-mbstring");
